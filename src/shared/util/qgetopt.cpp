...
bool GetOpt::parse( bool untilFirstSwitchOnly )
{
    ...
    {
      QStringList::ConstIterator it = args.isEmpty() ? args.end() : --args.end();
      const QStringList::ConstIterator  end = args.end();
      while ( it != end )
      {
        stack.push( *it );
        if (it==args.begin())
	...
	case StartState:
		...
		currOpt = opt;
		currType = t;
	    } else if ( opt.type == OShortSwitch ) {
		setSwitch( opt );
		setOptions.insert( opt.lname, 1 );
		setOptions.insert( QString( QChar( opt.sname ) ), 1 );
	    } else if ( opt.type == OShortOptSwitch ) {
		setSwitch( opt );
		setOptions.insert( opt.lname, 1 );
		setOptions.insert( QString( QChar( opt.sname ) ), 1 );
                state = OptionalState;
	...
	if ( untilFirstSwitchOnly && (opt.type == OSwitch || opt.type== OShortSwitch || opt.type == OOptSwitch || opt.type == OShortOptSwitch) )
	    return true;
...
//following function added by stefan:
void GetOpt::addOptSwitch( const QString &lname, bool *b, bool def)
{
    Option opt( OOptSwitch, 0, lname );
    opt.boolValue = b;
    addOption( opt );
    // ### could do all inits at the beginning of parse()
    *b = def;
...
void GetOpt::addShortSwitch( char s, const QString &lname, bool *b )
{
    Option opt( OShortSwitch, s, lname );
    opt.boolValue = b;
    addOption( opt );
    // ### could do all inits at the beginning of parse()
    *b = false;
...
//following function added by stefan:
void GetOpt::addShortOptSwitch( char s, const QString &lname, bool *b, bool def)
{
    Option opt( OShortOptSwitch, s, lname );
    opt.boolValue = b;
    addOption( opt );
    // ### could do all inits at the beginning of parse()
    *b = def;
...
{
    assert( (o.type == OSwitch || o.type== OShortSwitch || o.type == OOptSwitch || o.type == OShortOptSwitch) );
    *o.boolValue = true;
}
